...
// DAGDAG SA teachers_db.js:

SimpleLogin.saveTeacherScan = function(teacherId, studentData) {
    if (!teacherId || !studentData) return null;
    
    try {
        // 1. Save to TEACHER'S PERSONAL RECORDS
        const teacherKey = `teacher_${teacherId}_records`;
        const teacherRecords = JSON.parse(localStorage.getItem(teacherKey) || '[]');
        
        const record = {
            studentName: studentData.name,
            studentNumber: studentData.number,
            studentStrand: studentData.strand,
        ...
        localStorage.setItem(teacherKey, JSON.stringify(teacherRecords));
        
        // 2. Save to SHARED STUDENT-TEACHER RECORDS (para makita ng student)
        const sharedKey = `student_${studentData.number}_scans`;
        const studentScans = JSON.parse(localStorage.getItem(sharedKey) || '[]');
        
        // Get teacher info
        const teacher = this.getCurrentTeacher();
        const teacherName = teacher ? teacher.fullName : 'Unknown Teacher';
        
        studentScans.unshift({
            teacherName: teacherName,
            teacherId: teacherId,
            teacherUsername: teacher ? teacher.username : 'unknown',
            scanTime: new Date().toISOString(),
            subject: "Attendance" // pwedeng lagyan ng subject later
        });
        
        localStorage.setItem(sharedKey, JSON.stringify(studentScans));
        ...
        console.error('Error saving teacher scan:', error);
        return null;
    }
};